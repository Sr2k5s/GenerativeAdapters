...
INSTRUCTION_TEMPLATE = (
    "You are a data-to-text NLG system.\n"
    "Convert the following RDF-style triples into ONE short, fluent English sentence (<= 20 words)\n"
    "that describes the information.\n\n"
    "Return ONLY a valid JSON object with EXACTLY one key:\n\n"
    '{{\"sentence\": \"<your sentence here>\"}}\n\n'
    "Rules:\n"
    "- Do NOT include explanations, reasoning, bullet points, code, or markdown.\n"
    "- Do NOT add any keys other than \"sentence\".\n"
    "- Do NOT write anything before or after the JSON object.\n\n"
    "Triples:\n"
    "{triples}\n\n"
    "JSON:\n"
)
def _linearize_triple(triple: Any) -> str:
    """
    Turn a DART triple into 'subj | pred | obj'.
    Triple may be a list/tuple or a dict.
    """
...
def preprocess_dart(example: Dict) -> Dict:
    """
    Build src_texts from the tripleset with a strict JSON instruction;
    target from target / text / references.
    """
    triples = example.get("tripleset", [])
    triple_strings: List[str] = [_linearize_triple(t) for t in triples]
    ...
    # Instructional prompt for Gemma
    src_text = INSTRUCTION_TEMPLATE.format(triples=triples_block)

    # target: prefer 'target', then 'text', then first 'references'
    tgt = example.get("target", None)
    if tgt is None:
